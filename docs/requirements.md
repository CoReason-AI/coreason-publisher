...
* **Council Snapshot:** Generates council\_manifest.lock.
  * *Content:* {"proposer\_a": "gpt-4-0613", "judge": "claude-3-opus-20240229"}.
...
* **Stage 1: Proposal (SRE):**
  ...
  * Locks the Foundry Draft (status: PENDING\_SRB).
* **Stage 2: Review (SRB):**
...
Bundling: Publisher detects adapter\_model.bin (400MB). It initializes LFS tracking and moves it to models/.
...
Finding: The council\_manifest.lock shows the agent was tested using gpt-3.5 instead of the required gpt-4.
...
├── council\_manifest.lock  \# LLM Dependency Lockfile
...
│   └── adapter\_model.bin  \# Fine-Tuned Weights
├── data/                  \# GIT LFS MANAGED
│   ├── bec\_corpus.jsonl   \# The Verification Dataset
│   └── synthesis\_map.json \# Data Provenance
└── evidence/
    └── assay\_report.json  \# Pass/Fail Metrics
...
3. **CoA Templating:** Use Jinja2 to render the CERTIFICATE.md file dynamically from the assay\_report.json.
