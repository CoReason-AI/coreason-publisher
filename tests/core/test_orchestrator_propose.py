...
def test_propose_release_success(
    ...
    deps["electronic_signer"].create_signature.assert_called_once_with(workspace_path, mock_user_context)
    deps["git_local"].add_all.assert_called_once()
    deps["git_local"].commit.assert_called_once_with("Commit Message")
    deps["electronic_signer"].send_audit_to_veritas.assert_called_once_with(mock_user_context, "dummy-hash", "SRE")
    ...
    deps["foundry_client"].submit_for_review.assert_called_once_with(
        "draft-1", type="release", user_context=mock_user_context
    )
    deps["git_provider"].post_comment.assert_called_once()


def test_propose_release_mr_failure(
    tmp_path: Path, mock_dependencies: dict[str, Any], mock_user_context: UserContext
) -> None:
...
def test_propose_release_lfs_verification_fails(
    tmp_path: Path, mock_dependencies: dict[str, Any], mock_user_context: UserContext
) -> None:
    """
