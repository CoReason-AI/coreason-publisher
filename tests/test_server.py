...
def test_propose_success(client, mock_orchestrator):
    payload = {
        "project_id": "proj-123",
        "draft_id": "draft-456",
        "bump_type": "patch",
...
def test_propose_failure_400(client, mock_orchestrator):
...
def test_propose_failure_502(client, mock_orchestrator):
...
def test_health_success(client, mock_orchestrator):
    response = client.get("/health")
    assert response.status_code == 200
    assert response.json()["status"] == "healthy"

def test_health_failure_lfs(client, mock_orchestrator):
